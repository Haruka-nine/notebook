...
![[Pasted image 20221018131016.png]]
...
![[Pasted image 20221019071902.png]]
...
![[Pasted image 20221019075542.png]]
### 对象头（Header）
...
![[Pasted image 20221019080032.png]]


## 对象的访问定位

![[Pasted image 20221019080711.png]]

JVM是如何通过栈帧中的对象引用访问到其内部的对象实例呢？

![[Pasted image 20221019080730.png]]

### 句柄访问

![[Pasted image 20221019080756.png]]

reference中存储稳定句柄地址，对象被移动（垃圾收集时移动对象很普遍）时只会改变句柄中实例数据指针即可，reference本身不需要被修改

### 直接指针（HotSpot采用）

![[Pasted image 20221019080832.png]]
...
![[Pasted image 20221019082248.png]]

### 直接缓存区

使用NIO时，操作系统划出的直接缓存区可以被java代码直接访问，只有一份。NIO适合对大文件的读写操作。
![[Pasted image 20221019082322.png]]
...
![[Pasted image 20221019082417.png]]
...
![[Pasted image 20221019091259.png]]
...
JVM的主要任务是负责装载**字节码到其内部**，但字节码并不能够直接运行在操作系统之上，因为字节码指令并非等价于本地机器指令，它内部包含的仅仅只是一些能够被JVM所识别的字节码指令、符号表，以及其他辅助信息。
![[Pasted image 20221019091647.png]]
...
![[Pasted image 20221019092011.png]]
从外观上来看，所有的Java虚拟机的执行引擎输入，输出都是一致的：输入的是字节码二进制流，处理过程是字节码解析执行的等效过程，输出的是执行过程。

## Java代码编译和执行过程

![[Pasted image 20221019092851.png]]


大部分的程序代码转换成物理机的目标代码或虚拟机能执行的指令集之前，都需要经过上图中的各个步骤
![[Pasted image 20221019093113.png]]
Java代码编译是由Java源码编译器（**前端编译器**）来完成，流程图如下所示：

![[Pasted image 20221019093216.png]]
...
![[Pasted image 20221019093450.png]]
...
在汇编语言中，用**助记符**（Mnemonics）代替**机器指令的操作码**，用**地址符号（Symbol）或标号（Label）** 代替指令或操作数的地址。在不同的硬件平台，汇编语言对应着不同的机器语言指令集，通过汇编过程转换成机器指令。
...
![[Pasted image 20221019095215.png]]
...
![[Pasted image 20221019095259.png]]
...
![[Pasted image 20221019095331.png]]
...
![[Pasted image 20221019105809.png]]
